...
vignette: >
  %\VignetteIndexEntry{useful_packages}
...
* [asciiSetupReader](https://jacobkap.github.io/asciiSetupReader/) Reads in data that comes as fixed-width ASCII files (.txt or .dat) that have an .sps (SPSS) or .sas (SAS) setup file explaining to the software how to read that file. These file combinations are sometimes referred to as .txt+.sps, .txt+.sas, .dat+.sps, .dat+.sas. 
...
* [fastDummies](http://jacobkap.github.io/fastDummies/) - Easy creation of dummy variables. Note you can generally avoid *having* to make dummy variables by just doing `factor(variableyouwantdummiesof)` in your regression.
